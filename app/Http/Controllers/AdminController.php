...
class AdminController extends Controller {
    ...
    public function dashboard(Request $request) {
        ...
        try {
            ...
            $wallet['agent_debit'] = Agent::select(DB::raw('SUM(CASE WHEN wallet_balance<0 THEN wallet_balance ELSE 0 END) as total_debit'))->get()->toArray();
            $wallet['agent_credit'] = Agent::select(DB::raw('SUM(CASE WHEN wallet_balance>=0 THEN wallet_balance ELSE 0 END) as total_credit'))->get()->toArray();
            ...
            return view('admin.dashboard', compact('providers', 'agent', 'provider', 'scheduled_rides', 'users', 'rides', 'totalRides', 'user_cancelled', 'provider_cancelled', 'cancel_rides', 'revenue', 'wallet'));
        } catch (Exception $e) {
    ...
    public function settings() {
        ...
        $site_settings = unserialize(session('site_admin_settings'));
    ...
    public function settings_store(Request $request) {
        ...
        if (!file_exists($config)) {
            ...
            $data = "<?php 
				...
				return array(
					...
					'site_email_logo' => '',
					'site_icon' => '',
					'site_copyright' => '© 2019 Thinkin Service',
					...
					'sos_number' => '911',
					'contact_number' => '',
					'contact_email' => 'admin@thinkin.com',
					'environment' => '',
					'ios_push_password' => '',
					'android_push_key' => '',
        ...
        if (!$request->has('manual_request')) {
            ...
            $change_content = str_replace($search_text, $value_text, $change_content);
            $admin_settings['manual_request'] = '0';
        }
        ...
        if (!$request->has('broadcast_request')) {
            ...
            $change_content = str_replace($search_text, $value_text, $change_content);
            $admin_settings['broadcast_request'] = '0';
        ...
        if ($request->hasFile('site_icon')) {
            ...
            $change_content = str_replace($search_text, $value_text, $change_content);
            $admin_settings['site_icon'] = $site_icon;
        }
        ...
        if ($request->hasFile('site_logo')) {
            ...
            $change_content = str_replace($search_text, $value_text, $change_content);
            $admin_settings['site_logo'] = $site_logo;
        }
        ...
        if ($request->hasFile('site_email_logo')) {
            ...
            $change_content = str_replace($search_text, $value_text, $change_content);
            $admin_settings['site_email_logo'] = $site_email_logo;
        }

        file_put_contents($config, $change_content);

        session()->put('site_admin_settings', serialize($admin_settings));

        // dd($this->admin_settings);

        // session('reload', true);

        return back()->with('flash_success', 'Settings updated successfully!');
        // return redirect()->route('admin.settings')->with('flash_success', 'Settings updated successfully!');
    }
    ...
    public function settings_payment_store(Request $request) {
        ...
        if (!$request->has('daily_target')) {
            ...
            if (($request->has('cash') == 0 && $request->has('card') == 0) && $request->has('payumoney') == 0 && $request->has('voucher') == 0 && $request->has('debit_machine') == 0 && $request->has('paypal') == 0 && $request->has('paypal_adaptive') == 0 && $request->has('braintree') == 0 && $request->has('paytm') == 0) {
                ...
                return back()->with('flash_error', 'At least one payment method must be activated.
');
            }
        }
    ...
    public function profile_update(Request $request) {
        ...
        try {
            ...
            return redirect()->back()->with('flash_success', 'Updated profile!');
        } catch (Exception $e) {
    ...
    public function help() {
        try {
            $str = '{"content":"<p>We&#39;d like to thank you for deciding to use our script. We enjoyed creating it and hope you enjoy using it to achieve your goals :)&nbsp;If you want something changed to suit&nbsp;your venture&#39;s needs better, drop us a line: info@thinkindragon.com<\/p>\r\n"}';
    ...
    public function statement_agent() {
        ...
        try {
            ...
            $Agents = Agent::paginate($this->perpage);

            $pagination = (new Helper)->formatPagination($Agents);
            ...
            return view('admin.providers.agent-statement', compact('Agents', 'pagination'))->with('page', 'Agents Statement');
        } catch (Exception $e) {
            return back()->with('flash_error', 'Something went wrong!');
        }
    }
    ...
    public function transferlist(Request $request) {
        ...
        if ($croute == 'admin.agenttransfer')
            $pendinglist = $pendinglist->with('agent');
        else
    ...
    public function search(Request $request) {
        ...
        if ($sflag == 1)
            ...
            $queries = Agent::where('name', 'LIKE', $term . '%')->take(5)->get();
    ...
    public function fare(Request $request) {
        ...
        try {
            ...
            $responsedata = 0 ;//$response->calculateFare($request->all(), 1);
